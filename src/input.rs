...
impl InputContext {
    pub fn new(
        ...
        let applet_res = hid_srv
            .get()
            .create_applet_resource(sf::ProcessId::from(aruid))?;
        ...
        hid_srv
            .get()
            .set_supported_npad_style_set(sf::ProcessId::from(aruid), supported_tags)?;
        hid_srv.get().set_supported_npad_id_type(
            sf::ProcessId::from(aruid),
            sf::Buffer::from_array(controllers),
        )?;
        hid_srv.get().activate_npad(sf::ProcessId::from(aruid))?;
    ...
    pub fn get_touch_data(&mut self, touch_index: u32) -> Result<TouchData> {
        unsafe {
            ...
            result_return_unless!(
                (touch_index as u64) < (*touch_entry).count,
                rc::ResultInvalidTouchIndex
            );
...
impl Drop for InputContext {
    fn drop(&mut self) {
        ...
        let _ = self
            .hid_service
            .get()
            .deactivate_npad(sf::ProcessId::from(self.aruid));
